...
const ASSIGNMENT = {
  ...
  description: 'Complete the Kinematic Velocity Calculator practice to master the primary linear kinematic equation (v_f = v_o + at). Solve for final velocity, initial velocity, acceleration, and elapsed time across all 3 tiers with Desmos calculator verification.\n\nActivity Link: https://rrmudry.github.io/kinematic_velocity_calculator/dist/index.html',
  maxPoints: 10,
  dueDate: { year: 2026, month: 9, day: 23 }, // Sep 22 6:00 PM PDT is Sep 23 01:00 UTC
  dueTime: { hours: 1, minutes: 0 },
  activityUrl: 'https://rrmudry.github.io/kinematic_velocity_calculator/dist/index.html'
};

async function postAssignments() {
  console.log('====================================================');
  console.log(`🚀 Posting Assignment: "${ASSIGNMENT.title}"`);
  console.log(`📊 Points: ${ASSIGNMENT.maxPoints} pts | Due: Tomorrow (9/22) at 6:00 PM PDT`);
  ...
  if (db) {
    try {
      const docNames = [
        ...
        'kinematic_velocity_calculator',
        'unit2_day17_kinematic_velocity_calculator'
      ];
      ...
      for (const docName of docNames) {
        await db.collection('gradest_assignments').doc(docName).set({
          ...
          title: ASSIGNMENT.title,
          assignmentId: 'kinematic_velocity_calculator',
