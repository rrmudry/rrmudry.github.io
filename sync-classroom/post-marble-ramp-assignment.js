...
const ASSIGNMENT = {
  ...
  description: 'Complete the Marble Ramp Motion & Graphing Lab. Investigate the motion of a marble rolling down an inclined ruler ramp across a flat table surface, collect precision timing data with the digital stopwatch, construct interactive position vs. time and velocity vs. time graphs by clicking to create data points, calculate slopes and areas under the curve, and submit your complete lab report for credit.\n\nActivity Link: https://rrmudry.github.io/Unit_2/marble_ramp_lab/index.html',
  maxPoints: 10,
  dueDate: { year: 2026, month: 9, day: 17 }, // Sep 16 6:00 PM PDT is Sep 17 01:00 UTC
  dueTime: { hours: 1, minutes: 0 },
  activityUrl: 'https://rrmudry.github.io/Unit_2/marble_ramp_lab/index.html'
};

async function postAssignments() {
  console.log('====================================================');
  console.log(`🚀 Posting Assignment: "${ASSIGNMENT.title}"`);
  console.log(`📊 Points: ${ASSIGNMENT.maxPoints} pts | Due: 9/16 at 6:00 PM PDT`);
  ...
  if (db) {
    try {
      const docName = ASSIGNMENT.title.trim();
      ...
      // Also set for "Marble_Ramp_Lab" document for 1:1 match
      await db.collection('gradest_assignments').doc('Marble_Ramp_Lab').set({
        assignmentName: ASSIGNMENT.title,
        title: ASSIGNMENT.title,
        assignmentDetails: ASSIGNMENT.description,
        description: ASSIGNMENT.description,
      ...
      console.log(`💾 Linked deployment metadata to Firestore "gradest_assignments/Marble_Ramp_Lab"`);
    } catch (dbErr) {
