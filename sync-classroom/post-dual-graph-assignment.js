...
const ASSIGNMENT = {
  title: 'Dual-Graph Velocity vs Time Practice',
  description: 'Complete the Dual-Graph Velocity vs Time Practice studio activity to master translating between position-time and velocity-time graphs, calculating displacement as area under the curve, identifying velocity intervals, and reading multi-segment trips.\n\nActivity Link: https://rrmudry.github.io/Unit_2/dual_graph_studio/index.html',
  maxPoints: 10,
  dueDate: { year: 2026, month: 9, day: 16 }, // Sep 15 6:00 PM PDT is Sep 16 01:00 UTC
  dueTime: { hours: 1, minutes: 0 },
  activityUrl: 'https://rrmudry.github.io/Unit_2/dual_graph_studio/index.html'
};

async function postAssignments() {
  console.log('====================================================');
  console.log(`🚀 Posting Assignment: "${ASSIGNMENT.title}"`);
  console.log(`📊 Points: ${ASSIGNMENT.maxPoints} pts | Due: 9/15 at 6:00 PM PDT`);
  ...
  if (db) {
    try {
      ...
      // Also set for "Dual_Graph_Studio" document for 1:1 match
      await db.collection('gradest_assignments').doc('Dual_Graph_Studio').set({
        assignmentName: ASSIGNMENT.title,
        title: ASSIGNMENT.title,
        assignmentDetails: ASSIGNMENT.description,
        description: ASSIGNMENT.description,
      ...
      console.log(`💾 Linked deployment metadata to Firestore "gradest_assignments/Dual_Graph_Studio"`);
    } catch (dbErr) {
