 ...
 * Marble Ramp Motion & Graphing Lab
 * Enforces single-replication capture: each captured time can fill the active replication.
 */
class PrecisionStopwatch {
  ...
  init() {
    ...
    // Spacebar shortcut to Start/Stop stopwatch (ignoring text inputs)
  ...
  reset(silent = false) {
    ...
    this.updateDisplay(0);
    this.updateRecordButtonState();
    if (!silent && window.labSound) window.labSound.playClick();
  }
  ...
  recordCurrentTime() {
    ...
    const recordedSeconds = this.getSeconds();
    if (recordedSeconds <= 0) return;

    if (window.labEngine && window.labEngine.receiveStopwatchTime) {
      const accepted = window.labEngine.receiveStopwatchTime(recordedSeconds);
      if (accepted) {
        if (window.labSound) window.labSound.playSuccess();
        // Reset stopwatch back to 00:00.00 ready for the next trial
        this.reset(true);
      }
    }
  }
