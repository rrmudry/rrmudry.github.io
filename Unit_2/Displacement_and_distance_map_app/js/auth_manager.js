/**
 * AuthManager - Google Sign-In & Gradebook Sync
 * Restricts access to Orange USD accounts and submits highest score to Firestore.
 */
const ASSIGNMENT_ID = "Fantasy_Map_Distance_Displacement";
...
class AuthManager {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        if (email.endsWith('@orangeusd.org') || email === 'ryan.mudry@gmail.com') {
          ...
          // Automatically calibrate the realm path specifically for this student
          if (window.fantasyMap) {
            window.fantasyMap.setUserSeedFromId(this.studentId);
            if (window.traveler) window.traveler.refreshTrail();
            if (window.questEngine) window.questEngine.renderStep(window.questEngine.currentStep);
          }
        } else {
  ...
  async submitScore(percentageScore, questReport = {}) {
    ...
    try {
      ...
      parentRef.set({
        assignment_name: "Fantasy Map: Distance vs Displacement",
