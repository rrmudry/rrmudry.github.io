...
html_path = r'c:\Users\rmudry\Unit_4_Dashboard\Digitalcules\index.html'

with open(html_path, 'r', encoding='utf-8') as f:
    text = f.read()

# Extract from // --- DIGITALCULES DATA & EVOLUTIONARY LOGIC --- to the end
idx = text.find('// --- DIGITALCULES DATA & EVOLUTIONARY LOGIC ---')
if idx != -1:
    logic_code = text[idx:]
else:
    logic_code = text # fallback

# Remove any remaining imports if they somehow got in
logic_code = re.sub(r'import React.*?[\n\r]', '', logic_code, flags=re.MULTILINE)
logic_code = re.sub(r'import \{.*?lucide-react\';[\n\r]', '', logic_code, flags=re.MULTILINE|re.DOTALL)

# Replace export default function App
logic_code = logic_code.replace('export default function App()', 'function App()')
...
<head>
    ...
    <title>Digitalcules: Fossil Record Lab</title>
...
final_content = html_boilerplate.replace('{LOGIC_CODE}', logic_code)
