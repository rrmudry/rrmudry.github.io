        ...
        // Extract hooks from React global
        const { useState, useRef, useEffect } = React;

        // --- INLINE SVG COMPONENTS (replacing lucide-react) ---
        ...
        const Play = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><polygon points="6 3 20 12 6 21 6 3"></polygon></svg>;
        const RotateCcw = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path></svg>;
...
// Determine which traits CAN mutate this generation
const eligibleKeys = [];

// 1. Cosmetic Traits (Can drift, but we lower the odds so structural traits get a chance to evolve)
if (Math.random() < 0.3) eligibleKeys.push('color');
if (Math.random() < 0.2) eligibleKeys.push('shape');
// 2. Additive Traits (Once acquired, they never go away)
if (newTraits.pattern === 'solid') eligibleKeys.push('pattern'); // solid -> spots/stripes
    if (!newTraits.tail) eligibleKeys.push('tail'); // no tail -> tail
    if (newTraits.eyes < 2) eligibleKeys.push('eyes'); // 0 -> 1 -> 2

        // 3. Progressive Limbs
        if (newTraits.limbs === 'none') eligibleKeys.push('limbs_to_fins');
        else if (newTraits.limbs === 'fins') eligibleKeys.push('limbs_to_legs');

        // If organism is fully evolved/maxed out, no mutations happen
        if (eligibleKeys.length === 0) return newTraits;

        // STRICT RULE: Only exactly 1 mutation happens per generation to ensure clear visual lineages!
        ...
        // Helper to check trait equality
        ...
        // Ensure every parent has at least 1 child to keep the tree connected
        const childrenAssignment = prevEraNodes.map(p => ({ parent: p, childrenCount: 1 }));
        let remaining = eraInfo.count - prevEraNodes.length;

        // Distribute remaining children randomly among the parents
        while(remaining > 0) {
...
// If we are stuck in a loop trying to find a unique combination
// force a cosmetic mutation so specimens on the same stratum don't look identical
if (attempts > 5) {
            if (Math.random() > 0.5) {
            ...
            attempts++;
            } while (isDuplicate && attempts < 20); const childNode={ id: childId, era: eraInfo.mya, traits: childTraits
                }; newSpecies.push(childNode); currentEraNodes.push(childNode); newEdges.push({ from:
                assignment.parent.id, to: childId }); } }); prevEraNodes=currentEraNodes; }); return { speciesDatabase:
                newSpecies, correctEdges: newEdges }; }; // --- SVG GENERATOR COMPONENT --- const CreatureSVG=({ traits,
                size=60 })=> {
                ...
                <svg width={size} height={size} viewBox="0 0 100 100" className="drop-shadow-md overflow-visible">
                    <g transform="translate(50, 50)">
                        {/* Tail */}
                        {tail && (
                        <path d="M 0 35 Q 0 60 -20 70" fill="none" stroke={color} strokeWidth="8"
                            strokeLinecap="round" />
                        )}
                        ...
                        {/* Body Shape */}
                        {shape === 'round' &&
                        <circle cx="0" cy="0" r="35" fill={color} />}
                        {shape === 'oval' &&
                        <ellipse cx="0" cy="0" rx="45" ry="30" fill={color} />}
                        {shape === 'long' &&
                        <ellipse cx="0" cy="0" rx="25" ry="45" fill={color} />}
                ...
                // --- GEOLOGICAL TEXTURE DEFINITIONS ---
                const SvgDefinitions = () => (
                <svg width="0" height="0" className="absolute pointer-events-none">
                    <defs>
                        {/* 74 MYA: Shale (Dark gray bedrock with strict horizontal pressure lines) */}
                        ...
                        {/* 50 MYA: Conglomerate (Brown with varying embedded rocks/pebbles) */}
                        ...
                        {/* 30 MYA: Sandstone (Tan with dense stippled dots) */}
                        ...
                        {/* 15 MYA: Siltstone (Light tan with wavy horizontal sediment lines) */}
                        <pattern id="siltstone" width="60" height="30" patternUnits="userSpaceOnUse">
                            <rect width="60" height="30" fill="#e7e5e4" />
                            <path d="M0 8 Q 15 12, 30 8 T 60 8" stroke="#d6d3d1" fill="none" strokeWidth="2"
                                opacity="0.8" />
                            <path d="M0 22 Q 15 26, 30 22 T 60 22" stroke="#a8a29e" fill="none" strokeWidth="2"
                                opacity="0.6" />
                        </pattern>

                        {/* 0 MYA: Topsoil (Rich dark brown with organic specks) */}
                ...
                function App() {
                // Initialize game data once per session, uniquely generated
                const [gameData, setGameData] = useState(() => generateTreeData());
                const { speciesDatabase, correctEdges } = gameData;

                const [placedNodes, setPlacedNodes] = useState({}); // { id: { x: percentage, y: eraIndex } }
                const [connections, setConnections] = useState([]); // [ { from: id, to: id } ]
                const [timePhase, setTimePhase] = useState(0); // 0 = Present only, 4 = All eras
                ...
                // Clear validation styling when user edits the board
                useEffect(() => {
                setValidation(null);
                }, [connections, placedNodes]);

                // Filter available species based on the "Time Machine" phase
                // Phase 0: only era 0. Phase 1: era 0, 15. etc.
                ...
                // Validate era! Organisms MUST be placed in their correct geological stratum
                ...
                // Distinguish between a drag and a click based on movement distance
                if (dragStartPos.current) {
                const dx = e.clientX - dragStartPos.current.x;
                const dy = e.clientY - dragStartPos.current.y;

                if (Math.abs(dx) < 5 && Math.abs(dy) < 5) { // It was a click! if (connectingFrom) { // Complete a
                    connection if (connectingFrom !==id) { const fromNode=speciesDatabase.find(s=> s.id ===
                    connectingFrom);
                    const toNode = speciesDatabase.find(s => s.id === id);

                    if (fromNode.era === toNode.era) {
                    setModalConfig({ type: 'alert', message: "Cannot connect organisms from the same era directly. They
                    must share an ancestor." });
                    } else {
                    ...
                    if (timePhase < eras.length - 1) setTimePhase(prev=> prev + 1);
                        ...
                        setModalConfig({
                        type: 'confirm',
                        message: "Are you sure you want to clear your connections and reset the board? Your organisms
                        ...
                        setModalConfig({
                        type: 'confirm',
                        message: "Are you sure you want to generate a completely new evolutionary tree? This will erase
                        all your current progress.",
                        onConfirm: () => {
                        ...
                        // Make sure all organisms for current eras are placed
                        const allExpectedNodesPlaced = unplacedSpecies.length === 0;
                        if (!allExpectedNodesPlaced) {
                        setModalConfig({ type: 'alert', message: "Please place all discovered specimens on the board
                        before checking your work." });
                        return;
                        }

                        // Helper to ensure connection strings are always ordered "Older_Newer" for safe comparisons
                        ...
                        // Check drawn connections against the master key
                        ...
                        // Count how many total edges they *should* have drawn by this phase
                        ...
                        isPerfect: incorrectCount === 0 && missingCount <= 0 }); }; const takeScreenshot=async (e)=> {
                            e.stopPropagation();
                            setIsCapturing(true);
                            try {
                            // Dynamically load html2canvas to avoid external dependency issues
                            ...
                            // Capture the main workspace area
                            ...
                            setModalConfig({ type: 'alert', message: "Sorry, there was an error capturing the
                            screenshot." });
                            } finally {
                            ...
                            // Calculate pixel coordinates for the SVG lines based on percentage state
                            const getLineCoordinates = (fromId, toId) => {
                            const fromNode = placedNodes[fromId];
                            const toNode = placedNodes[toId];
                            if (!fromNode || !toNode) return null;

                            // Y is based on the eraIndex (row)
                            // There are eras.length rows. Each row is 100% / eras.length height.
                            // Center of row is (index + 0.5) * (100 / eras.length)
                            ...
                            return (
                            <div
                                className="flex flex-col h-screen bg-slate-50 font-sans overflow-hidden text-slate-800">
                                <SvgDefinitions />

                                {/* HEADER */}
                                <header
                                    className="bg-slate-800 text-white p-4 shadow-md flex justify-between items-center z-10">
                                    <div>
                                        <h1 className="text-2xl font-bold flex items-center gap-2">
                                            <Clock className="w-6 h-6 text-emerald-400" />
                                            Digitalcules: Fossil Record Lab
                                        </h1>
                                        <p className="text-slate-300 text-sm">Trace the evolutionary history using
                                            morphology and strata.</p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div
                                            className="flex items-center gap-2 bg-slate-700 p-2 rounded-lg shadow-inner">
                                            <span
                                                className="text-sm font-semibold uppercase tracking-wider text-slate-300">Time
                                                Machine:</span>
                                            <div className="flex gap-1">
                                                {eras.map((era, idx) => (
                                                <div key={era.mya} className={`h-3 w-8 rounded-full transition-colors
                                                    ${idx <=timePhase
                                                    ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]'
                                                    : 'bg-slate-600' }`} title={era.label} />
                                                ))}
                                            </div>
                                            {timePhase < eras.length - 1 ? ( <button onClick={advanceTime}
                                                ...
                                                <span className="ml-2 text-xs text-emerald-400 font-bold px-2">All Eras
                                                    Found</span>
                                                )}
                                        </div>

                                        <button onClick={()=> setShowInstructions(true)} className="p-2
                                ...
                                <div className="flex flex-1 overflow-hidden">
                                    ...
                                    {/* LEFT SIDEBAR: FIELD NOTEBOOK / TRAY */}
                                    <aside
                                        className="w-80 bg-white border-r border-slate-200 shadow-lg flex flex-col z-10">
                                        <div
                                            className="p-4 bg-slate-100 border-b border-slate-200 flex justify-between items-center">
                                            <h2 className="font-bold text-lg text-slate-700">Specimen Tray</h2>
                                            <div className="flex gap-1">
                                                <button onClick={resetActivity}
                                                    className="p-1.5 text-slate-500 hover:text-rose-500 hover:bg-slate-200 rounded transition-colors"
                                                    title="Clear Canvas & Reset Connections">
                                                    <RotateCcw className="w-5 h-5" />
                                                </button>
                                                <button onClick={generateNewLab}
                                                    className="p-1.5 text-slate-500 hover:text-purple-600 hover:bg-slate-200 rounded transition-colors"
                                                    title="Generate New Evolutionary Tree">
                                        ...
                                        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6">
                                            ...
                                            return (
                                            <div key={era.mya}
                                                className="border border-slate-200 rounded-lg p-3 bg-slate-50 shadow-sm">
                                                <h3
                                                    className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 border-b border-slate-200 pb-1">
                                                    {era.label}
                                                </h3>
                                                <div className="flex flex-wrap gap-2 justify-center">
                                                    {eraSpecies.map(species => (
                                                    <div key={species.id} draggable onDragStart={(e)=>
                                                        handleDragStartTray(e, species)}
                                                        className="cursor-grab hover:scale-110 hover:shadow-lg
                                                        transition-all bg-white p-2 rounded-xl border border-slate-200
                                                        shadow-sm"
                                                        title={`Drag to ${era.label} stratum`}
                                                        >
                                                        <CreatureSVG traits={species.traits} size={50} />
                                                        <div
                                                            className="text-[10px] text-center text-slate-400 mt-1 font-mono">
                                                            {species.id}</div>
                                                    </div>
                                                    ))}
                                                </div>
                                        ...
                                        <div className="p-4 bg-blue-50 border-t border-blue-100">
                                            <h4
                                                className="font-bold text-blue-800 text-sm mb-2 flex items-center gap-1">
                                                <LinkIcon className="w-4 h-4" /> Tools
                                            </h4>
                                            <div className="text-xs text-blue-700 space-y-2">
                                                <p><strong>To connect:</strong> Click one organism, then click another.
                                                </p>
                                    ...
                                    {/* RIGHT AREA: PHYLOGENETIC CANVAS */}
                                    <main id="lab-workspace"
                                        className="flex-1 bg-slate-100 relative overflow-hidden flex flex-col">
                                        {/* Canvas container with absolute positioning for free X movement, but strict Y
                                        rows */}
                                        <div ref={canvasRef} className="flex-1 relative w-full h-full" style={{
                                            backgroundImage: 'linear-gradient(rgba(0,0,0,0.05) 1px, transparent 1px)' ,
                                            backgroundSize: `100% ${100 / eras.length}%` }} onPointerDown={()=>
                                            setConnectingFrom(null)}
                                            >
                                            {/* Validation Feedback Banner (Z-50 to stay well above everything, with
                                            explicit Close button) */}
                                            {validation && (
                                            <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-50 px-5 py-3
                                                rounded-full shadow-2xl font-bold text-sm text-white flex items-center
                                                gap-4 transition-all border-2 ${validation.isPerfect
                                                ? 'bg-emerald-500 border-emerald-400' : 'bg-rose-500 border-rose-400'
                                                }`}>
                                                <span>
                                                    {validation.isPerfect
                                                    ? "Perfect tree! You successfully navigated convergent evolution."
                                                    : `Checked: ${validation.correctCount} correct |
                                                    ${validation.incorrectCount} incorrect | ${validation.missingCount}
                                                    missing`}
                                                </span>
                                                <div className="flex items-center gap-2 border-l border-white/30 pl-4">
                                                    <button onClick={takeScreenshot} disabled={isCapturing}
                                                        ...
                                                        <span className="hidden sm:inline">{isCapturing ? 'Saving...' :
                                                            'Save Image'}</span>
                                                    </button>
                                                    <button onClick={(e)=> {
                                                        e.stopPropagation();
                                            ...
                                            {/* Render Strata Rows FIRST so they sit behind lines */}
                                            {eras.map((era, index) => (
                                            <div key={era.mya}
                                                ...
                                                style={{ height: `${100 / eras.length}%`, top: `${index * (100 /
                                                eras.length)}%`, }} onDragOver={handleDragOverCanvas} onDrop={(e)=>
                                                ...
                                                <span
                                                    className="relative z-10 text-slate-800 font-bold uppercase tracking-widest text-sm bg-white/70 backdrop-blur px-3 py-1 rounded shadow-sm border border-white/50">
                                                    {era.label} Stratum
                                                </span>
                                            </div>
                                            ))}

                                            {/* Draw Lines via SVG overlay (z-10 so they are clickable above strata) */}
                                            <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
                                                {connections.map((conn, idx) => {
                                                const coords = getLineCoordinates(conn.from, conn.to);
                                                if (!coords) return null;

                                                // Determine stroke color based on validation state
                                                ...
                                                emerald (correct) or red (incorrect)
                                                }
                                                ...
                                                <g key={`${conn.from}-${conn.to}`}
                                                    ...
                                                    onPointerDown={(e)=> {
                                                    e.stopPropagation();
                                                    setConnections(connections.filter(c => c.from !== conn.from || c.to
                                                    !== conn.to));
                                                    }}
                                                    >
                                            ...
                                            <div key={id} onPointerDown={(e)=> handleNodePointerDown(e, id)}
                                                ...
                                                <div className="relative group">
                                                    <div
                                                        className="bg-white rounded-full p-2 shadow-lg border-2 border-slate-200">
                                                        <CreatureSVG traits={species.traits} size={60} />
                                                    </div>

                                                    {/* Tooltip & Controls */}
                                                    <div
                                                        className="absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs py-1 px-3 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 flex items-center gap-2">
                                                        <span>ID: {id}</span>
                                                        <button onPointerDown={(e)=> { e.stopPropagation();
                                                            handleRemoveNode(id); }}
                                                            className="pointer-events-auto bg-red-500 hover:bg-red-600
                                                            p-1 rounded ml-2"
                                ...
                                <div
                                    ...
                                    <div
                                        ...
                                        <div className="p-6 overflow-y-auto text-slate-700 space-y-4">
                                            <p>
                                                In this simulation, you are a taxonomist and paleontologist. Your goal
                                                is to construct a <strong>phylogenetic tree</strong> (a family tree)
                                                showing how a group of fictional organisms evolved over 74 million
                                                years.
                                            </p>
                                            ...
                                            <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r">
                                                ...
                                                <p className="text-sm text-amber-900">
                                                    Start by dragging the <strong>Present Day (0 MYA)</strong> organisms
                                                    from the tray onto the top row of the canvas. Group them
                                                    horizontally based on shared physical traits (morphology) like body
                                                    shape, patterns, and limbs.
                                                </p>
                                            </div>

                                            <div className="bg-emerald-50 border-l-4 border-emerald-400 p-4 rounded-r">
                                                <h3 className="font-bold text-emerald-800 mb-1">Phase 2: The Fossil
                                                    Record</h3>
                                                <p className="text-sm text-emerald-900">
                                                    Once you've grouped the living species, click <strong>"Dig
                                                        Deeper"</strong> in the top bar. This acts like a time machine,
                                                    revealing older fossil strata (15 MYA, 30 MYA, etc.). Drag these
                                                    ancestors to their correct row.
                                                </p>
                                            </div>

                                            <div
                                                className="bg-purple-50 border-l-4 border-purple-400 p-4 rounded-r mt-4">
                                                <h3 className="font-bold text-purple-800 mb-1">Beware of Convergent
                                                    Evolution!</h3>
                                                <p className="text-sm text-purple-900">
                                                    Just because two organisms share a trait (like legs or a tail)
                                                    doesn't mean they are closely related. Similar traits can evolve
                                                    independently in completely different family lines! If a connection
                                                    seems right but doesn't fit the fossil record, break it and look for
                                                    an older ancestor.
                                                </p>
                                            </div>

                                            <h3 className="font-bold text-lg mt-4 border-b pb-2">How to use the tools:
                                            </h3>
                                            <ul className="list-disc pl-5 space-y-2 text-sm">
                                                <li><strong>Drag and Drop:</strong> Drag organisms from the tray to
                                                    their corresponding Era row on the canvas.</li>
                                                <li><strong>Organize:</strong> Drag placed organisms left or right to
                                                    align lineages.</li>
                                                <li><strong>Connect:</strong> Click an ancestor, then click a descendant
                                                    to draw an evolutionary line.</li>
                                                <li><strong>Disconnect:</strong> Click directly on a dashed line to
                                                    remove it.</li>
                                                <li><strong>Remove:</strong> Hover over an organism on the canvas and
                                                    click the red trash can to return it to the tray.</li>
                                            </ul>

                                            <p className="italic text-sm text-slate-500 mt-4">
                                                *Hint: Sometimes the fossil record will force you to redraw the
                                                connections you originally guessed!
                                            </p>
                                        </div>

                                        <div className="p-4 bg-slate-100 border-t border-slate-200 flex justify-end">
                                            <button onClick={()=> setShowInstructions(false)}
                                                className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2
                                ...
                                {/* CUSTOM MODAL (Replaces window.alert/confirm) */}
                                {modalConfig && (
                                <div
                                    className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
                                    <div
                                        className="bg-white rounded-xl shadow-2xl max-w-sm w-full overflow-hidden flex flex-col">
                                        <div className={`p-4 text-white font-bold flex items-center gap-2
                                            ${modalConfig.type==='alert' ? 'bg-amber-500' : 'bg-rose-500' }`}>
                                        ...
                                        <div
                                            className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                                            ...
                                            <button onClick={()=> {
                                                ...
                                                ${modalConfig.type === 'alert' ? 'bg-amber-500 hover:bg-amber-600' :
