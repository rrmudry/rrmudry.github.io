    ...
    <section id="view-register" class="max-w-md w-full glass p-8 md:p-10 rounded-[2.5rem] border border-white/10 relative z-10 shadow-2xl transition-all duration-500">
      <div class="text-center space-y-6">
        ...
        <div class="space-y-4 pt-2">
          <button id="btn-google-signin" type="button" onclick="handleGoogleSignIn()" class="w-full bg-white text-slate-900 py-4 rounded-2xl font-black uppercase tracking-widest hover:scale-[1.02] hover:bg-slate-100 transition-all flex items-center justify-center gap-3 shadow-2xl">
      ...
      <div class="flex flex-col lg:flex-row gap-4 items-start w-full">
         ...
         <div class="flex-grow w-full min-w-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-stretch">
              ...
              <!-- LEFT PANEL: Question Card & Badges -->
              <div class="md:col-span-7 flex flex-col gap-3">
                
                <!-- Word Problem Card -->
                <div class="glass rounded-xl p-4 border border-white/5 flex-grow flex flex-col justify-center relative min-h-[110px]">
                  <!-- Audio Speech Button -->
                  ...
                  <p id="question-text" class="text-base md:text-lg font-semibold leading-relaxed text-slate-100 pr-8 text-center">
                    <!-- Inline values and unknown will be rendered here dynamically -->
                  </p>
                </div>
      
                <!-- Selection helper text -->
                <div id="selection-helper" class="text-center text-[10px] text-slate-400 font-semibold h-3.5 select-none">
                  <!-- Selected item details -->
                </div>
      
                <!-- Equation Arranging Board: v_f = v_o + a * t -->
                <div class="glass rounded-xl p-4 border border-white/5 flex flex-col items-center justify-center">
                  
                  <!-- Standard Kinematic Formula Template: v_f = v_o + a * t -->
                  <div class="flex items-center justify-center gap-1.5 sm:gap-2.5 text-2xl sm:text-3xl font-mono text-slate-500 flex-wrap py-2">
                    
                    <!-- vf Slot (Final Velocity) -->
                    <div 
                      data-slot="vf" 
                      class="drag-target w-24 h-20 sm:w-28 sm:h-24 bg-slate-950/80 rounded-xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-sm font-semibold select-none cursor-pointer transition-all"
                      onclick="handleSlotClick(this)"
                    >
                      <span class="text-slate-500 text-xl sm:text-3xl font-mono select-none">v<sub>f</sub></span>
                      <span class="text-[8px] sm:text-[9px] text-slate-600 font-sans tracking-wider uppercase font-semibold select-none mt-0.5">final vel</span>
                    </div>
      
                    <span class="font-sans text-slate-400 font-bold select-none text-xl sm:text-2xl px-0.5">=</span>
                    ...
                    <div 
                      ...
                      onclick="handleSlotClick(this)"
                    >
                      <span class="text-slate-500 text-xl sm:text-3xl font-mono select-none">v<sub>o</sub></span>
                      <span class="text-[8px] sm:text-[9px] text-slate-600 font-sans tracking-wider uppercase font-semibold select-none mt-0.5">initial vel</span>
                    </div>
                    ...
                    <div 
                      ...
                      onclick="handleSlotClick(this)"
                    >
                      <span class="text-slate-500 text-xl sm:text-3xl font-mono select-none">a</span>
                      <span class="text-[8px] sm:text-[9px] text-slate-600 font-sans tracking-wider uppercase font-semibold select-none mt-0.5">accel</span>
                    </div>

                    <span class="font-sans text-slate-400 font-bold select-none text-xl sm:text-2xl px-0.5">&middot;</span>
                    ...
                    <div 
                      ...
                      onclick="handleSlotClick(this)"
                    >
                      <span class="text-slate-500 text-xl sm:text-3xl font-mono select-none">t</span>
                      <span class="text-[8px] sm:text-[9px] text-slate-600 font-sans tracking-wider uppercase font-semibold select-none mt-0.5">time</span>
                    </div>
              ...
              <!-- RIGHT PANEL: Action Panel / Visual Feedback -->
              <div class="md:col-span-5 flex flex-col gap-3">
                ...
                <div id="calc-input-helper" class="glass rounded-xl p-4 border border-white/5 flex flex-col hidden">
                  ...
                  <div class="my-2.5 flex flex-col gap-2">
                    <input 
                      ...
                      placeholder="Enter or paste result..." 
                      oninput="syncManualInput(this.value)"
                      class="w-full bg-slate-950/85 text-white py-2.5 px-3 rounded-xl border border-white/10 text-center font-bold tracking-wide text-lg focus:border-sky-500 focus:outline-none transition-all mono placeholder:font-normal placeholder:text-slate-600"
                    >
                    <button 
                      id="btn-paste-desmos-input"
                      type="button" 
                      onclick="pasteDesmosResult()" 
                      class="w-full py-2.5 px-3 bg-gradient-to-r from-sky-600 to-cyan-600 hover:from-sky-500 hover:to-cyan-500 text-white rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-md hover:scale-[1.01] active:scale-95 border border-sky-400/30 cursor-pointer"
                      title="Paste latest calculated value from Desmos"
                    >
                      <i data-lucide="clipboard-paste" class="w-4 h-4 text-cyan-200"></i>
                      <span>Paste from Desmos</span>
                    </button>
                  </div>
                ...
                <div class="flex flex-col gap-2">
                  <button id="btn-check-answer" onclick="checkCurrentAnswer()" class="w-full bg-sky-600 hover:bg-sky-500 text-white font-black py-2.5 px-4 rounded-xl transition-all shadow-lg text-xs uppercase tracking-wider">
                    Check Answer
                  </button>
                  <button onclick="resetWorkspace()" class="w-full bg-slate-900 hover:bg-slate-800 text-slate-300 font-bold py-2.5 px-4 rounded-xl transition-all border border-white/5 text-xs uppercase tracking-wider">
                    Reset Workspace
                  </button>
                  <button id="btn-next-question" onclick="nextQuestion()" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-2.5 px-4 rounded-xl transition-all shadow-lg text-xs uppercase tracking-wider hidden">
                    Next Question
                  </button>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 mt-1">
                  <button onclick="toggleHelpModal(true)" class="bg-slate-900 hover:bg-slate-800 text-slate-300 py-2.5 rounded-xl border border-white/5 font-semibold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-1.5">
                    <i data-lucide="help-circle" class="w-4 h-4"></i> Help Guide
                  </button>
                  <button onclick="toggleCalculatorModal(true)" class="bg-slate-900 hover:bg-slate-800 text-slate-300 py-2.5 rounded-xl border border-white/5 font-semibold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-1.5">
  ...
  <script>
      ...
      const renderSlot = (slotName) => {
        ...
        if (item === 'unknown') {
          ...
          if (state.currentLevel >= 2 && slotName === q.solveFor) {
            ...
            \`;
            slotEl.className = "drag-target w-24 h-20 sm:w-28 sm:h-24 bg-slate-900 border-2 border-slate-700 rounded-xl flex flex-col items-center justify-center transition-all scale-102";
          } else {
            slotEl.innerHTML = \`
            ...
            \`;
            slotEl.className = \`drag-target w-24 h-20 sm:w-28 sm:h-24 \${color} border-2 rounded-xl flex flex-col items-center justify-center transition-all scale-102\`;
          }
        } else if (item) {
          ...
          \`;
          slotEl.className = "drag-target w-24 h-20 sm:w-28 sm:h-24 bg-slate-950/80 rounded-xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-slate-500 select-none cursor-pointer transition-all";
        }
      };
    ...
    function checkCurrentAnswer() {
      ...
      if (state.currentLevel === 1) {
        ...
        \`;
        unknownSlotEl.className = "drag-target w-24 h-20 sm:w-28 sm:h-24 bg-emerald-950/20 border-2 border-emerald-500 rounded-xl flex flex-col items-center justify-center transition-all scale-105";
