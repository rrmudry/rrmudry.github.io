 ...
 * Allows any Unit Dashboard, Lesson Plan, Activity, or Web App across rrmudry.github.io
 * to automatically pull, query, and render interactive NGSS Standards badges & details.
 */
...
(function () {
    ...
    const NGSSHelper = {
        /**
         * Ensure standards data is loaded
         */
        ensureDataLoaded: function (callback) {
            if (window.NGSS_STANDARDS_DATA && window.NGSS_STANDARDS_DATA.length > 0) {
                if (callback) callback();
                return;
            }

            // Dynamically load standards-data.js
            const script = document.createElement('script');
            script.src = `${rootPath}ngss-explorer/standards-data.js`;
            script.onload = () => {
                if (callback) callback();
            };
         ...
         * @param {HTMLElement|string} targetContainer - HTMLElement or element ID string
         */
        renderStandardsBanner: function (codesArray, targetContainer) {
            this.ensureDataLoaded(() => {
                ...
                let html = `
                    <div class="ngss-banner-card" style="background: rgba(15, 23, 42, 0.7); backdrop-filter: blur(12px); border: 1px solid rgba(0, 243, 255, 0.25); border-radius: 16px; padding: 1.25rem; margin-top: 1rem; margin-bottom: 1rem;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
                            <h4 style="font-size: 0.85rem; font-weight: 700; color: #00f3ff; text-transform: uppercase; letter-spacing: 1px; display: flex; align-items: center; gap: 0.4rem; margin: 0;">
                                <span>📚</span> NGSS Target Standards Alignment
                            </h4>
                            <a href="${rootPath}ngss-explorer/index.html" target="_blank" style="font-size: 0.75rem; color: #94a3b8; text-decoration: none; font-weight: 600;">Open Full Explorer ↗</a>
                        </div>
                ...
                modalEl.innerHTML = `
                    <div style="background: rgba(15, 23, 42, 0.95); border: 1px solid rgba(0, 243, 255, 0.4); border-radius: 20px; padding: 2rem; max-width: 650px; width: 100%; max-height: 85vh; overflow-y: auto; box-shadow: 0 20px 50px rgba(0,0,0,0.8); position: relative; color: #f8fafc;">
                        ...
                        <div style="margin-top: 1.5rem; text-align: right;">
                            <a href="${rootPath}ngss-explorer/index.html" target="_blank" style="display: inline-block; background: linear-gradient(135deg, #00f3ff 0%, #3b82f6 100%); color: #020617; font-weight: 700; padding: 0.6rem 1.2rem; border-radius: 10px; text-decoration: none; font-size: 0.85rem;">Open in NGSS Explorer ↗</a>
                        </div>
                    </div>
                `;
         ...
         * Scans the document for data attributes:
         * 1. [data-ngss="HS-PS2-1,HS-PS2-2"] -> Renders standards banner automatically!
         * 2. [data-ngss-is="IS1"] -> Renders all standards in IS1 automatically!
         * 3. [data-ngss-code="HS-PS2-1"] -> Makes element click-to-open modal trigger!
         */
        autoInit: function () {
            this.ensureDataLoaded(() => {
                // 1. Elements with data-ngss="CODES"
                document.querySelectorAll('[data-ngss]').forEach(el => {
                    const codes = el.dataset.ngss.split(',').map(s => s.trim()).filter(Boolean);
                    if (codes.length > 0) {
                        this.renderStandardsBanner(codes, el);
                    }
                });

                // 2. Elements with data-ngss-is="IS_CODE"
                document.querySelectorAll('[data-ngss-is]').forEach(el => {
                    const isCode = el.dataset.ngssIs.trim();
                    const stds = this.getStandardsByIS(isCode);
                ...
                // 3. Elements with data-ngss-code="CODE" (standalone buttons or inline triggers)
                document.querySelectorAll('[data-ngss-code]').forEach(el => {
                    const code = el.dataset.ngssCode.trim();
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => NGSSHelper.autoInit());
    } else {
        NGSSHelper.autoInit();
    }
