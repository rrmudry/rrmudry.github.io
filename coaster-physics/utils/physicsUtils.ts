...
export const pointsToPathSegments = (points: Point[]) => {
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const cp1x = pStart.x + ((pEnd.x - pPrev.x) / 6) * tension;
    const cp1y = pStart.y + ((pEnd.y - pPrev.y) / 6) * tension;

    const cp2x = pEnd.x - ((pNext.x - pStart.x) / 6) * tension;
    const cp2y = pEnd.y - ((pNext.y - pStart.y) / 6) * tension;
