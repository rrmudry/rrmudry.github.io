...
const IdEntryScreen: React.FC<{onIdSubmit: (id: string) => void}> = ({ onIdSubmit }) => {
    ...
    const handleIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (/^[0-9]*$/.test(value) && value.length <= 6) {
            setId(value);
            if (error) { // Clear error when user types
                setError('');
            }
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-100">
            <div className="bg-white p-8 rounded-lg shadow-lg text-center max-w-sm w-full">
                ...
                <p className="text-slate-600 mt-2 mb-6">Please enter your 6-digit Student ID to begin.</p>
                <input
                    type="text" // Keep as text to control input manually
                    value={id}
                    onChange={handleIdChange}
                    onKeyDown={handleKeyDown} // Add Enter key submission
                    placeholder="6-digit Student ID"
                    className={`w-full px-4 py-2 border rounded-md ${error ? 'border-red-500' : 'border-slate-300'}`}
                    maxLength={6}
                    autoComplete="off"
                />
                ...
                <button
                    ...
                    disabled={id.length !== 6} // Disable if not 6 digits
                    className="w-full mt-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed"
                >
                    Start
                </button>
            </div>
